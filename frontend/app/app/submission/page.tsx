      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button onClick={lock} disabled={!editable || !status.submission}>
              Lock &amp; enter the Gauntlet
            </Button>
            {message && (
              <p className="border border-accent/40 bg-accent/10 px-3 py-1.5 font-mono text-[11px] uppercase tracking-wider text-accent-strong">
                [ SYS ] {message}
              </p>
            )}
          </div>
      ...
      {progress && status.job && status.job.status !== "COMPLETED" && (
        <Card>
          <CardContent className="flex items-center gap-3">
            <span className="h-2 w-2 bg-accent led-pulse" />
            <p className="font-mono text-sm font-medium uppercase tracking-wider text-ink">
              Gauntlet live · {progress}
            </p>
          </CardContent>
        </Card>
      )}
          ...
          <CardContent>
            <div className="grid gap-3 sm:grid-cols-4">
              {[
                ...
                <div key={label as string} className="module px-4 py-3">
                  <p className="font-mono text-[10px] font-medium uppercase tracking-wider text-ink-soft">
                    {label as string}
                  </p>
                  <p className="mt-1 font-mono text-lg font-bold tabular-nums text-ink">{v as number}</p>
                  <div className="progress-bar mt-1.5">
                    <div className="progress-fill" style={{ width: `${v as number}%` }} />
                  </div>
                </div>
              ))}
            </div>
            <p className="mt-3 font-mono text-[11px] uppercase tracking-wider text-ink-soft tabular-nums">
          ...
          <CardContent>
            <div className="grid gap-3 sm:grid-cols-4">
              <div className="border border-accent/30 bg-accent/10 px-4 py-3">
                <p className="font-mono text-[10px] font-medium uppercase tracking-wider text-accent-strong">Gauntlet</p>
                <p className="mt-1 font-mono text-lg font-bold tabular-nums text-ink">{status.score.gauntletScore}</p>
              </div>
              <div className="border border-warn/30 bg-warn-soft px-4 py-3">
                <p className="font-mono text-[10px] font-medium uppercase tracking-wider text-warn">Casino mult.</p>
                <p className="mt-1 font-mono text-lg font-bold tabular-nums text-ink">×{status.score.casinoMultiplier}</p>
              </div>
              <div className="module px-4 py-3">
                <p className="font-mono text-[10px] font-medium uppercase tracking-wider text-ink-soft">Discipline</p>
                <p className="mt-1 font-mono text-lg font-bold tabular-nums text-ink">{status.score.disciplineScore}</p>
              </div>
              <div className="border border-accent bg-accent px-4 py-3">
                <p className="font-mono text-[10px] font-medium uppercase tracking-wider text-white/70">Final</p>
                <p className="mt-1 font-mono text-lg font-bold tabular-nums text-white glow-text">
